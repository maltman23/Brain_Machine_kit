...
[Mitch note:  the above link is no longer available, but here is another link with good info for putting an Arduino into low-power modes:
...
resets.  This hardware could probably hold all this data in memory, but since the original system
...
If we choose a central pitch of 100 Hz, 
...
/***************************************************
  ...
  pin  5  - Left ear
  pin  6  - Right ear
  pin  9  - Right eye LED1
  pin 10  - Left eye LED2
...
void setup()  { 
  ...
  leftEar.begin(leftEarLow);        // Enables the Tone library to play a pitch on the leftEarLow output pin
...
void wakeUpNow()        // here the interrupt is handled after wakeup (if we enabled an interrupt)
{
  ...
  //
  // [Mitch:  For this sketch, there should never be an interrupt to wake up the microcontroller]
}
